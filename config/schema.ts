 ...
 * 环境类型守卫
 */
export const isEnvironment: TypeGuard<Environment> = (value: unknown): value is Environment => {
 ...
 * 日志级别类型守卫
 */
export const isLogLevel: TypeGuard<'debug' | 'info' | 'warn' | 'error'> = (value: unknown): value is 'debug' | 'info' | 'warn' | 'error' => {
    return typeof value === 'string' &&
 ...
 * 日志格式类型守卫
 */
export const isLogFormat: TypeGuard<'json' | 'pretty' | 'simple'> = (value: unknown): value is 'json' | 'pretty' | 'simple' => {
    return typeof value === 'string' &&
        ['json', 'pretty', 'simple'].includes(value);
};

/**
 * 字符串转换器
 */
export const stringTransformer: ConfigTransformer<string> = (value: string) => value.trim();
 ...
 * 布尔值转换器
 */
export const booleanTransformer: ConfigTransformer<boolean> = (value: string) => {
 ...
 * 环境转换器
 */
export const environmentTransformer: ConfigTransformer<Environment> = (value: string) => {
 ...
 * 日志级别转换器
 */
export const logLevelTransformer: ConfigTransformer<'debug' | 'info' | 'warn' | 'error'> = (value: string) => {
    if (!isLogLevel(value)) {
 ...
 * 日志格式转换器
 */
export const logFormatTransformer: ConfigTransformer<'json' | 'pretty' | 'simple'> = (value: string) => {
 ...
 * 温度参数验证器
 */
export const temperatureValidator: ConfigValidator<number> = (value: number) => {
 ...
 * 数据库 URL 验证器
 */
export const databaseUrlValidator: ConfigValidator<string> = (value: string) => {
...
export const jwtSecretValidator: ConfigValidator<string> = (value: string) => {
    return value.length >= 32 && !value.includes('placeholder') && !value.includes('your_');
};
...
export const passwordValidator: ConfigValidator<string> = (value: string) => {
    return value.length >= 8 && !value.includes('placeholder') && !value.includes('your_');
};
...
export const apiKeyValidator: ConfigValidator<string> = (value: string) => {
    return value.length >= 16 && !value.includes('placeholder') && !value.includes('your_');
};
...
export const defaultProviders = {
        ...
        switch (environment) {
            ...
            case 'test':
                return {
                    ...baseConfig,
                    NODE_ENV: 'test',
                    DATABASE_URL: 'postgresql://postgres:test_password_123@localhost:15433/fastify_react_app_test',
    ...
    development: (environment: Environment): Partial<DevelopmentConfig> => {
        ...
        return {
            ...
            HOT_RELOAD: isDev,
            TEST_DATABASE_URL: 'postgresql://postgres:test_password@localhost:15433/fastify_react_app_test',
    ...
    app: (environment: Environment): Partial<AppConfig> => ({
        ...
        DESCRIPTION: '现代化全栈应用',
        JWT_EXPIRES_IN: environment === 'test' ? '1h' : environment === 'ci' ? '30m' : '7d'
    })
};
...
export const validationRules = {
    ...
    business: {
        PAGINATION_LIMIT: { validator: (v: number) => v > 0 && v <= 1000, required: true },
        PAGINATION_MAX_LIMIT: { validator: (v: number) => v > 0 && v <= 1000, required: true },
    ...
    llm: {
        ...
        MAX_TOKENS: { validator: (v: number) => v > 0 && v <= 10000, required: true },
 ...
 * 验证配置对象
 */
export function validateConfig(config: Partial<Config>, environment: Environment): ConfigValidationResult {
    ...
    if (config.business) {
        for (const [key, rule] of Object.entries(validationRules.business)) {
            ...
            } else if (value !== undefined && value !== null && !(rule.validator as any)(value)) {
                errors.push(createValidationError(
                    `business.${key}`,
                    'invalid_value',
    ...
    // 生产环境特殊验证
    if (environment === 'production') {
        if (!config.security?.JWT_SECRET || config.security.JWT_SECRET.includes('placeholder')) {
            errors.push(createValidationError(
                'security.JWT_SECRET',
                'invalid_value',
        ...
        if (!config.security?.DB_PASSWORD || config.security.DB_PASSWORD.includes('placeholder')) {
            errors.push(createValidationError(
