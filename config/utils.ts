...
export class ConfigValidator {
    ...
    static validateConfigCompleteness(config: Config): string[] {
        ...
        if (!config.security.LLM_API_KEY || config.security.LLM_API_KEY.includes('placeholder')) {
    ...
    static validateProductionConfig(config: Config): string[] {
        ...
        for (const { key, value } of sensitiveVars) {
            if (value.includes('placeholder') || value.includes('your_') || value.includes('dev_')) {
                issues.push(`${key} contains placeholder value in production environment`);
            }
        }
...
export class ConfigFileManager {
    ...
    static generateAllEnvFiles(outputDir: string): void {
        const environments: Environment[] = ['development', 'production', 'staging', 'test', 'ci'];
        ...
        console.log(`Generated ${environments.length} environment files in ${outputDir}`);
    }
...
export class ConfigComparator {
    ...
    static compare<T extends Record<string, any>>(
        ...
        const added = [...keys2].filter(key => !keys1.has(key) && !ignoreKeys.includes(key));
        const removed = [...keys1].filter(key => !keys2.has(key) && !ignoreKeys.includes(key));
        const common = [...keys1].filter(key => keys2.has(key) && !ignoreKeys.includes(key));
    ...
    static compareEnvironments(
        ...
        return {
            differences: [...comparison.added, ...comparison.removed, ...comparison.changed],
