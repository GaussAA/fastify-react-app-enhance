...
export function detectEnvironment(): Environment {
    ...
    if (env && ['development', 'production', 'staging', 'test', 'ci'].includes(env)) {
 ...
 * 构建安全配置
 */
function buildSecurityConfig(envVars: RawEnvVars, environment: Environment): SecurityConfig {
 ...
 * 构建环境配置
 */
function buildEnvironmentConfig(envVars: RawEnvVars, environment: Environment): EnvironmentConfig {
 ...
 * 构建开发配置
 */
function buildDevelopmentConfig(envVars: RawEnvVars, environment: Environment): DevelopmentConfig {
 ...
 * 构建应用配置
 */
function buildAppConfig(envVars: RawEnvVars, environment: Environment): AppConfig {
...
export function validateRequiredEnvVars(environment: Environment): void {
    ...
    if (environment === 'production') {
        ...
        if (placeholderVars.length > 0) {
            throw new Error(`生产环境不允许使用占位符: ${placeholderVars.join(', ')}`);
        }
    }
}
 ...
 * 获取配置摘要
 */
export function getConfigSummary(config: Config, includeSensitive = false): any {
    const summary: any = {
        environment: {
        ...
        security: includeSensitive ? config.security : {
