...
`;
    ...
    generateDockerCompose(outputPath = null) {
        const templatePath = join(this.projectRoot, 'infrastructure/docker/docker-compose.template.yml');
        if (!existsSync(templatePath)) {
        ...
        // 确定输出路径
        const finalOutputPath = outputPath || join(this.projectRoot, 'infrastructure/docker/docker-compose.yml');
