...
class RBACManager {
    ...
    async backupRBAC() {
        ...
        try {
            ...
            // 备份RBAC相关表
            const tables = ['users', 'roles', 'permissions', 'user_roles', 'role_permissions', 'audit_logs'];
            ...
            execSync(`pg_dump -h localhost -U postgres -d fastify_react_app -t ${tableList} > ${backupPath}`, {
                cwd: this.projectRoot,
                stdio: 'inherit',
            });
    ...
    async restoreRBAC(backupFile) {
        ...
        try {
            ...
            // 恢复数据
            execSync(`psql -h localhost -U postgres -d fastify_react_app < ${backupFile}`, {
                cwd: this.projectRoot,
                stdio: 'inherit',
            });
    ...
    async getAdminUserCount() {
        try {
            const result = execSync('npx prisma db execute --stdin', {
                cwd: this.apiDir,
                input: 'SELECT COUNT(*) FROM "users" u JOIN "user_roles" ur ON u.id = ur."userId" JOIN "roles" r ON ur."roleId" = r.id WHERE r.name = \'admin\';',
                stdio: 'pipe',
            });
            return parseInt(result.toString().trim());
        } catch (error) {
