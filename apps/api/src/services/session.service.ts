...
export class SessionManager extends EventEmitter {
     ...
     * 添加消息到会话
     */
    async addMessage(sessionId: string, message: Omit<ConversationMessage, 'id' | 'timestamp'>): Promise<SessionState | null> {
        const session = this.sessions.get(sessionId);
        if (!session) return null;
     ...
     * 更新会话上下文
     */
    async updateContext(sessionId: string, context: Record<string, any>): Promise<SessionState | null> {
        const session = this.sessions.get(sessionId);
        if (!session) return null;
    ...
    async restoreSession(sessionId: string): Promise<SessionState | null> {
        try {
            ...
            const session: SessionState = {
                ...
                conversationHistory: dbSession.conversationHistory as ConversationMessage[]
            };
...
export function getSessionManager(prisma?: PrismaClient, config?: Partial<SessionConfig>): SessionManager {
