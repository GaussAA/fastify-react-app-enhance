...
export class LLMServiceManager {
    ...
    validateConfig(config: LLMServiceConfig): { valid: boolean; errors: string[] } {
        ...
        if (config.defaultParams?.temperature !== undefined && (config.defaultParams.temperature < 0 || config.defaultParams.temperature > 2)) {
            errors.push('Temperature must be between 0 and 2');
        }

        if (config.defaultParams?.max_tokens !== undefined && config.defaultParams.max_tokens < 1) {
