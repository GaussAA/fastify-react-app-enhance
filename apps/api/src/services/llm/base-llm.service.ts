...
export abstract class BaseLLMService implements ILLMService {
  ...
  abstract chatStream(request: LLMChatRequest): AsyncIterable<LLMStreamResponse>;
  ...
  updateConfig(config: Partial<LLMServiceConfig>): void {
    ...
    this.logger.info('LLM service config updated', { provider: this.config.provider });
  }
  ...
  protected async withRetry<T>(
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        this.logger.warn(`LLM service retry attempt ${attempt + 1}/${retries}`, {
          provider: this.config.provider,
          context,
          delay,
          error: error.message
