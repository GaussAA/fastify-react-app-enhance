...
export class DialogueManager extends EventEmitter {
    ...
    constructor(sessionManager: SessionManager, config: Partial<DialogueConfig> = {}) {
        super();
     ...
     * 创建新的对话状态
     */
    async createDialogue(sessionId: string, initialState: string = 'greeting'): Promise<DialogueState> {
    ...
    async processUserInput(sessionId: string, userInput: string): Promise<{
        ...
        // 生成响应
        const response = await this.generateResponse(dialogue, intentResult, stateResult);

        // 更新对话状态
        await this.updateDialogueState(dialogue, intentResult, stateResult, response);

        // 记录对话轮次
        await this.recordDialogueTurn(dialogue, userInput, intentResult, response, stateResult);
    ...
    private async processStateMachine(
        ...
        // 查找匹配的转换
        const transition = currentState.transitions.find(t =>
            t.intent === intentResult.intent && intentResult.confidence >= t.confidence
        );
        if (!transition) {
            // 如果没有匹配的转换，保持当前状态
            return {
    ...
    private async generateResponse(
        ...
        const stateResponses = responses[stateResult.newState] || responses['general'];
        const randomResponse = stateResponses[Math.floor(Math.random() * stateResponses.length)];

        // 根据上下文个性化响应
        return this.personalizeResponse(randomResponse, dialogue.context, intentResult.entities);
    }
    ...
    private personalizeResponse(
        ...
        // 如果有用户名，使用用户名
        if (context.userName) {
            personalizedResponse = personalizedResponse.replace('你', context.userName);
        }
    ...
    async resumeDialogue(sessionId: string): Promise<string> {
        ...
        const response = resumeResponses[Math.floor(Math.random() * resumeResponses.length)];
    ...
    getDialogueStats(): {
        ...
        stats.averageTurnsPerDialogue = stats.totalDialogues > 0 ? totalTurns / stats.totalDialogues : 0;
...
class State {
    ...
    findTransition(intent: string, confidence: number): StateTransitionRule | undefined {
...
export function getDialogueManager(sessionManager?: SessionManager, config?: Partial<DialogueConfig>): DialogueManager {
