...
export const roleService = {
     ...
     * 为角色分配权限
     */
    async assignPermissionsToRole(roleId: number, permissionNames: string[], assignedBy?: number) {
        try {
            const role = await prisma.role.findUnique({
                where: { id: roleId },
            });
            ...
            logger.info(`Permissions assigned to role: ${role.name} -> ${permissionNames.join(', ')}`);
            return { success: true };
        } catch (error) {
