...
export const authService = {
     ...
     * 用户注册
     */
    async register(userData: CreateUserData, ipAddress?: string, userAgent?: string): Promise<AuthResult> {
        try {
            // 检查邮箱是否已存在
            ...
            // 创建用户会话
            await this.createUserSession(user.id, token, refreshToken, ipAddress, userAgent);
    ...
    async login(loginData: LoginData): Promise<AuthResult> {
        try {
            ...
            // 创建用户会话
            await this.createUserSession(user.id, token, refreshToken, loginData.ipAddress, loginData.userAgent);
