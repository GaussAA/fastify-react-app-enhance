...
export class AIIntegrationService extends EventEmitter {
    ...
    private async generateAIResponse(params: {
        ...
        const { userMessage, intent, dialogueResult, knowledgeResults, session, options } = params;

        // 构建上下文
        const context = this.buildContext(session, intent, knowledgeResults);
        ...
        try {
            ...
            if (llmResponse.choices && llmResponse.choices.length > 0) {
                ...
                // 根据意图和知识库结果优化响应
                response = this.optimizeResponse(response, intent, knowledgeResults, dialogueResult);
            ...
            // 回退到对话管理器的响应
            return dialogueResult.response || '抱歉，我遇到了一些技术问题，请稍后再试。';
        }
    }
     ...
     * 构建上下文
     */
    private buildContext(session: any, intent: any, knowledgeResults: any): Record<string, any> {
    ...
    private optimizeResponse(
        ...
        if (knowledgeResults && knowledgeResults.results.length > 0) {
            ...
            if (topResult.relevanceScore > 0.8) {
                // 高相关性结果，可以引用知识库内容
                optimizedResponse = this.enhanceWithKnowledge(optimizedResponse, topResult);
            }
        }

        // 根据意图调整响应风格
        optimizedResponse = this.adjustResponseStyle(optimizedResponse, intent.intent);
     ...
     * 调整响应风格
     */
    private adjustResponseStyle(response: string, intent: string): string {
        switch (intent) {
            case 'greeting':
    ...
    async getSystemHealth(): Promise<SystemHealth> {
        ...
        try {
            ...
            // 计算性能指标
            health.metrics.averageResponseTime = this.performanceMetrics.averageResponseTime;
            health.metrics.errorRate = this.performanceMetrics.totalRequests > 0
...
export function getAIIntegrationService(prisma?: PrismaClient, fastify?: any): AIIntegrationService {
