...
async function initializeAISystem() {
  ...
  try {
    ...
    console.log(`   - 活跃会话: ${health.metrics.activeSessions}`);
    console.log(`   - 平均响应时间: ${health.metrics.averageResponseTime}ms`);
    console.log(
      `   - 内存使用率: ${(health.metrics.memoryUsage * 100).toFixed(2)}%`
    );
  } catch (error) {
  ...
  const knowledgeEntries = [
    {
      title: '系统功能介绍',
      content:
        '本AI系统提供智能对话、意图识别、知识库查询等功能。支持多轮对话、会话管理和质量监控。',
      category: 'system',
      tags: ['系统', '功能', '介绍'],
    ...
    },
    {
      title: '如何使用对话功能',
      content:
        '用户可以直接发送消息与AI进行对话。系统会自动识别用户意图，提供相关回答。支持上下文理解和多轮对话。',
      category: 'usage',
      tags: ['使用', '对话', '指南'],
      metadata: { priority: 'high', version: '1.0' },
    },
    {
      title: '会话管理说明',
      content:
        '系统会自动管理用户会话，包括会话创建、超时处理、状态跟踪等。每个会话都有独立的上下文和状态。',
      category: 'session',
      tags: ['会话', '管理', '说明'],
    ...
    },
    {
      title: '意图识别功能',
      content:
        '系统能够识别用户的意图，包括问候、问题、请求、感谢等。基于规则和模式匹配进行意图识别。',
      category: 'intent',
      tags: ['意图', '识别', '功能'],
      metadata: { priority: 'medium', version: '1.0' },
    },
    {
      title: '知识库查询',
      content:
        '系统内置知识库，可以根据用户问题搜索相关信息。支持按类别、标签和相关性进行搜索。',
      category: 'knowledge',
      tags: ['知识库', '查询', '搜索'],
      metadata: { priority: 'medium', version: '1.0' },
    },
    {
      title: '质量监控机制',
      content:
        '系统会监控对话质量，包括响应时间、相关性、完整性等指标。提供质量报告和改进建议。',
      category: 'quality',
      tags: ['质量', '监控', '指标'],
    ...
    },
    {
      title: '常见问题解答',
      content:
        'Q: 如何开始对话？A: 直接发送消息即可开始对话。Q: 如何结束会话？A: 会话会自动超时或手动终止。',
      category: 'faq',
      tags: ['常见问题', '解答', 'FAQ'],
      metadata: { priority: 'high', version: '1.0' },
    },
    {
      title: '技术支持',
      content:
        '如遇到技术问题，请联系技术支持团队。系统提供详细的错误日志和性能监控信息。',
      category: 'support',
      tags: ['技术支持', '帮助', '联系'],
      metadata: { priority: 'medium', version: '1.0' },
    },
...
async function createSampleData() {
  ...
  console.log(`📝 示例用户创建/更新: ${sampleUser.name}`);
