...
export async function permissionRoutes(app: FastifyInstance) {
    ...
    app.post(
        '/',
        {
            preHandler: [authenticateToken, requirePermission('permission', 'create')],
            schema: {
                description: '创建新权限',
                tags: ['permissions'],
                summary: '创建权限',
        ...
        async (request, reply) => {
            try {
                const { name, displayName, description, resource, action } = request.body as {
                    name: string;
                    displayName: string;
                    description?: string;
                    resource: string;
    ...
    app.put(
        '/:id',
        {
            preHandler: [authenticateToken, requirePermission('permission', 'update')],
            schema: {
                description: '更新权限信息',
                tags: ['permissions'],
                summary: '更新权限',
        ...
        async (request, reply) => {
            try {
                ...
                const permission = await permissionService.updatePermission(id, updateData, updatedBy);
    ...
    app.delete(
        '/:id',
        {
            preHandler: [authenticateToken, requirePermission('permission', 'delete')],
            schema: {
                description: '删除权限',
                tags: ['permissions'],
                summary: '删除权限',
