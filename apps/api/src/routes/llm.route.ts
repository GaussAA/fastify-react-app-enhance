...
export async function llmRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<ChatRequest>('/chat', {
    ...
    try {
      const response = await llmManager.chat(request.body);
  ...
  fastify.post<StreamChatRequest>('/chat/stream', {
    ...
    try {
      ...
      const stream = llmManager.chatStream(request.body);
  ...
  fastify.get<ModelRequest>('/models/:modelId', {
    ...
    try {
      const model = await llmManager.getModel(request.params.modelId);
  ...
  fastify.post<ServiceConfigRequest>('/config', {
    ...
    try {
      const validation = llmManager.validateConfig(request.body);
      ...
      llmManager.setDefaultService(request.body);
