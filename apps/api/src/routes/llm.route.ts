...
import { LLMChatRequest, LLMServiceConfig } from '../types/llm.js';
...
export async function llmRoutes(fastify: FastifyInstance) {
  ...
  // 配置服务
  fastify.post<ServiceConfigRequest>(
    '/config',
    {
      schema: {
        ...
        body: {
          ...
          properties: {
            provider: {
              type: 'string',
              enum: ['deepseek', 'openai', 'anthropic', 'custom'],
            },
            apiKey: { type: 'string' },
            baseURL: { type: 'string' },
            model: { type: 'string' },
