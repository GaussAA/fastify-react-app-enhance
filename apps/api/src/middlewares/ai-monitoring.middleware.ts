...
export class AIMonitoringMiddleware {
  ...
  private recordMetrics(metrics: MonitoringMetrics) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(
        `📊 [${metrics.requestId}] ${metrics.method} ${metrics.url} - ${metrics.statusCode} (${metrics.responseTime}ms)`
      );
    }
  }
  ...
  private calculateStats(metrics: MonitoringMetrics[]) {
    ...
    return {
      ...
      averageAIProcessingTime:
        aiRequests.length > 0
          ? aiRequests.reduce((sum, m) => sum + (m.aiProcessingTime || 0), 0) /
            aiRequests.length
          : 0,
    };
  }
  ...
  private checkAnomalies(metrics: MonitoringMetrics[]) {
    ...
    if (slowRequests.length > 0) {
      console.warn(
        `⚠️ 发现 ${slowRequests.length} 个慢请求 (超过 ${threshold}ms)`
      );
    }
