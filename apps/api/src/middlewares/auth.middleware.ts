 ...
 * 支持多个权限的AND/OR逻辑
 */
export function requirePermissions(permissions: Array<{ resource: string, action: string }>, logic: 'AND' | 'OR' = 'AND') {
    return async (request: AuthenticatedRequest, reply: FastifyReply) => {
        if (!request.user) {
            return reply.status(401).send({
                success: false,
        ...
        const permissionChecks = await Promise.all(
            permissions.map(p => permissionService.hasPermission(request.user!.id, p.resource, p.action))
        );
