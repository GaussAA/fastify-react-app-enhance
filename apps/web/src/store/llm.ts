    ...
    persist(
        (set, get) => ({
                ...
                try {
                    ...
                    const backendSession = await aiSessionApiClient.createSession({
                        ...
                        options: {
                            ...
                            context: { title: title || `新对话 ${new Date().toLocaleString()}` }
                        }
                    });
            ...
            deleteSession: async (sessionId: string) => {
                ...
                try {
                    ...
                    set(state => ({
                        ...
                        backendSessionId: state.backendSessionId === sessionId ? null : state.backendSessionId
                    }));
                } catch (error: any) {
            ...
            sendMessage: async (message: string) => {
                const { currentSession, addMessage, setLoading, setError, config, currentUserId, backendSessionId } = get();
                ...
                try {
                    ...
                    const response = await aiSessionApiClient.processConversation({
                        ...
                        options: {
                            model: config.model,
                            temperature: config.temperature,
                            context: currentSession.messages.length > 0 ? {
            ...
            sendStreamMessage: async (message: string, onChunk: (chunk: string) => void) => {
                ...
                try {
                    ...
                    // 获取助手消息的索引
                    const assistantMessageIndex = get().currentSession!.messages.length - 1;
                    ...
                    // 流式处理完成后，更新最终消息内容
                    get().updateMessage(
                        assistantMessageIndex.toString(),
                        fullContent
                    );
                } catch (error: any) {
