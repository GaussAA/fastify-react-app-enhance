...
export function AnimationDemo({ transition, children, className = '' }: AnimationDemoProps) {
...
export function TransitionTester({ onTransitionChange, currentTransition }: TransitionTesterProps) {
    const transitions = [
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {transitions.map((transition) => (
                    <motion.button
                        ...
                        className={`px-4 py-2 rounded-lg font-medium transition-colors duration-200 ${currentTransition === transition.key
